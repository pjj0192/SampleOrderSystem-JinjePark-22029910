...
def test_list_confirmed_returns_only_confirmed_orders(tmp_path):
    service = make_service(tmp_path)
    still_reserved = service.reserve(sample_id="S-001", customer_name="A", quantity=10)
    to_confirm = service.reserve(sample_id="S-001", customer_name="B", quantity=10)
    service.approve(to_confirm.order_id)

    result = service.list_confirmed()

    assert [o.order_id for o in result] == [to_confirm.order_id]
    assert still_reserved.order_id not in [o.order_id for o in result]


def test_list_confirmed_returns_empty_list_when_none_confirmed(tmp_path):
    service = make_service(tmp_path)

    assert service.list_confirmed() == []
